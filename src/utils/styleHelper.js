...
import {COLOR_BORDER_BOTTOM} from 'constants/colors';
...
export function mapPropsToFontStyle(props) {
    ...
    const {bold, semi, medium} = props;
    const size = small ? 'small' : fit ? 'fit' : large ? 'large' : 'normal';
    const family = bold ? 'bold' : semi ? 'semi' : medium ? 'medium' : 'regular';
    const newProps = {
        fontSize: FONT_SIZE[`${size.toUpperCase()}`], fontFamily: FONT_FAMILY[`${family.toUpperCase()}`],
    };
    return newProps;
}
...
export const appShadow = StyleSheet.create({
    shadow: {
        shadowOffset: {
            height: 20, width: 0,
        }, shadowColor: 'rgba(194, 194, 194, 0.3)', shadowOpacity: 0.5, shadowRadius: 10, ...Platform.select({
            android: {
                borderBottomColor: COLOR_BORDER_BOTTOM, borderBottomWidth: 0.5,
            },
        }),
    },
});
