...
function TaskItem({item, index, getTaskListData, listTask}) {
    const {date, description, image, tkname, fullname, updated_time} = item || {};
    const [visible, setIsVisible] = useState(false);
    const [imageIndex, setImageIndex] = useState('');
    const END_POINT = AppConfigs.END_POINT;
    let imageData = [];
    //todo nên sửa lại ở server để trả về image luôn là mảng
    if (image && image !== '' && image !== '[undefined]' && image[0] !== 'f' && image[0] !== 'h' && image[0] !== 'i') {
    ...
    return (<View style={styles.container}>
        <View style={styles.content}>
            ...
            >
                ...
                <View
                    style={[
                        ...
                        {
                            backgroundColor: index === listTask?.length - 1 ? 'transparent' : '#D9DDE8'
                        },
                    ]}
                />
            </View>
            ...
            <View style={styles.right}>
                ...
                <AppText
                    ...
                    {`${moment(date).format(DATE_FORMAT['DD-MM-YYYY'])} - ${moment(timeConverter(updated_time)).format('HH:mm')}`}
                </AppText>
                <AppText
                    medium
                    style={{color: `#343333`}}>{`${description}`}</AppText>
                <ScrollView style={styles.imageContainer} horizontal>
                    {imageData.map((item, index) => (<TouchableOpacity
                        key={index}
                        onPress={() => {
                            setIsVisible(true)
                            setImageIndex(index)
                    ...
                    >
                        <Image source={item} style={styles.imageProduct} resizeMode={'cover'}/>
                    </TouchableOpacity>))}
                </ScrollView>
                <AppText style={{color: `rgba(160, 160, 160, 1)`}}> {`Ghi chép: ${fullname}`}</AppText>
                <TouchableOpacity
                    ...
                    onPress={() => {
                        Alert.alert('Xác nhận', 'Bạn chắc chắn muốn xóa nhiệm vụ này?', [{
                            text: 'Đồng ý', onPress: () => {
                                // xoa id_task
                                var config = {
                                    method: 'delete', url: 'https://egap.vn//task/' + item.id_task, headers: {
                                ...
                                axios(config)
                                    .then(function (response) {
                                        Alert.alert('Thông báo', 'Đã xóa nhiệm vụ thành công');
                                        getTaskListData();
                                        // onDelete();
        ...
        <ImageView
            ...
            FooterComponent={({imageIndex}) => (<View style={{flexDirection: 'row', justifyContent: 'center', marginBottom: 20}}>
